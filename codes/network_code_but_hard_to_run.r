...
# Cleanup
rm(ipc_epo, ipc_epo_dt, common_cols)
gc()
...
# Cleanup
rm(inv_pat, pat_inv_counts, multi_pat, coinv_el, inventor_nodes, comp)
gc()
...
# ===================================================================== --
# 25. Regions and Edge Construction Again ----
# ===================================================================== --
...
eu_ctry_codes  <- eu_ctry

regions_eu     <- regions %>% filter(ctry_code %in% eu_ctry)
eu_reg_codes   <- unique(regions_eu$reg_code)
eu_nuts3_codes <- eu_reg_codes

# Helper: build edges from one inventor–region file
make_edges_from_invfile <- function(path, eu_ctry, eu_reg_codes) {
  dt <- fread(
    path,
    sep      = "|",
    header   = TRUE,
    encoding = "UTF-8",
    select   = c("appln_id", "person_id", "reg_code", "ctry_code")
  ) %>%
    as_tibble() %>%
    rename_with(tolower)
  
  # Keep only EU inventors & valid regions
  dt <- dt %>%
    filter(
      ctry_code %in% eu_ctry,
      reg_code %in% eu_reg_codes,
      !is.na(reg_code),
      reg_code != ""
    )
  
  # Unique region per patent
  dt_unique <- dt %>%
    distinct(appln_id, reg_code)
  
  # All unordered region pairs per patent
  edges <- dt_unique %>%
    inner_join(
      dt_unique,
      by = "appln_id",
    ...
    filter(reg_code_from < reg_code_to) %>% # Avoid duplicates and self-loops
    count(from = reg_code_from, to = reg_code_to, name = "weight")
  
  edges
}

# Edges from EPO and PCT inventor–region files
edges_epo <- make_edges_from_invfile(
  "202401_EPO_Inv_reg.txt",
  eu_ctry,
  eu_reg_codes
)

edges_pct <- make_edges_from_invfile(
  "202401_PCT_Inv_reg.txt",
  eu_ctry,
  eu_reg_codes
)

# Combine and aggregate weights
edges_total <- bind_rows(edges_epo, edges_pct) %>%
  group_by(from, to) %>%
...
# Node table: TL3 regions that appear in edges
used_regions <- unique(c(edges_total$from, edges_total$to))

nodes_eu <- regions_eu %>%
  filter(reg_code %in% used_regions) %>%
  transmute(
    name         = reg_code,
    ctry_code,
    reg_label,
...
# igraph object: TL3 co-inventor region network (optional)
g_eu <- graph_from_data_frame(
  d        = edges_total,
  directed = FALSE,
  vertices = nodes_eu
)
...
print(g_eu)
print(summary(E(g_eu)$weight))

# Aggregate TL3 edges to TL2 / NUTS2 level
map_tl3_to_tl2 <- regions_eu %>%
  distinct(reg_code, up_reg_code)

# Attach TL2 to "from" and "to"
edges_tl2 <- edges_total %>%
  left_join(map_tl3_to_tl2, by = c("from" = "reg_code")) %>%
  rename(from_tl2 = up_reg_code) %>%
  left_join(map_tl3_to_tl2, by = c("to" = "reg_code")) %>%
  rename(to_tl2 = up_reg_code) %>%
  filter(!is.na(from_tl2), !is.na(to_tl2)) %>%
  # Drop within-region TL2 edges
  filter(from_tl2 != to_tl2) %>%
...
# Get NUTS2 shapes and centroids (Fixed corruption error here)
nuts2_sf <- gisco_get_nuts(
  nuts_level = 2,
  year       = 2010,
  resolution = "20",
  update_cache = FALSE # Forces re-download to fix corrupt file
) %>%
  filter(CNTR_CODE %in% eu_ctry) %>%
...
# Map
ggplot() +
  geom_sf(data = countries_laea, fill = "#f7f7f7", color = "#d0d0d0", size = 0.2) +
  geom_sf(data = edges_vis, aes(size = weight), colour = "grey40", alpha = 0.45) +
  scale_size_continuous(range = c(0.2, 0.9), guide = "none") +
  geom_sf(
    data = nodes_main_laea %>% mutate(has_patents = strength > 0),
    aes(color = log1p(strength), alpha = has_patents),
    size = 1.4
  ) +
  scale_alpha_manual(values = c(`FALSE` = 0.25, `TRUE` = 0.95), guide = "none") +
  scale_color_viridis_c(name = "Node strength\nlog(1 + sum of ties)", option = "C") +
  coord_sf(
...
# Flag helper
flag_dir <- "flags_png"
get_flag_img <- function(iso2) {
  iso2 <- tolower(iso2)
  fpath <- file.path(flag_dir, paste0(iso2, ".png"))
  if (file.exists(fpath)) return(png::readPNG(fpath))
  
  url <- sprintf("https://flagcdn.com/40x30/%s.png", iso2)
  tf  <- tempfile(fileext = ".png")
  res <- try(httr::GET(url, httr::write_disk(tf, overwrite = TRUE), quiet = TRUE), silent = TRUE)
  
  if (inherits(res, "try-error") || httr::status_code(res) != 200) {
    message("No flag for: ", iso2)
    return(NULL)
  }
  png::readPNG(tf)
}

# EPO inventor–region + IPC information
inv_epo <- fread(
  "202401_EPO_Inv_reg.txt",
  sep      = "|",
  header   = TRUE,
...
inv_epo_eu <- as_tibble(inv_epo) %>%
  filter(ctry_code %in% eu_ctry_codes, reg_code %in% eu_nuts3_codes)
...
inv_epo_eu <- inv_epo_eu %>%
  left_join(
    as_tibble(ipc_epo) %>% select(appln_id, app_year, IPC),
    by = "appln_id",
    relationship = "many-to-many"
  ) %>%
  mutate(ipc4 = substr(IPC, 1, 4)) %>%
  select(appln_id, person_id, reg_code, ctry_code, reg_share, inv_share, app_year, ipc4)
...
# Assuming 'inventor_panel' exists from previous steps
# load("base.RData") 
...
# ===================================================================== --
# 28. Structural Clustering and KS Distances ----
# ===================================================================== --

# Within-country standardisation of structural position
ctry_struct_z <- inventor_panel_geo %>%
  group_by(main_ctry) %>%
  mutate(
    deg_z_ctry    = (deg - mean(deg, na.rm = TRUE)) / sd(deg, na.rm = TRUE),
    kcore_z_ctry  = (kcore - mean(kcore, na.rm = TRUE)) / sd(kcore, na.rm = TRUE),
    constr_z_ctry = (constraint - mean(constraint, na.rm = TRUE)) / sd(constraint, na.rm = TRUE)
  ) %>%
  ungroup() %>%
  filter(is_broker_1sd) %>%
  group_by(main_ctry) %>%
  summarise(
    n_strong_brokers   = n(),
    mean_deg_z_ctry    = mean(deg_z_ctry, na.rm = TRUE),
    mean_kcore_z_ctry  = mean(kcore_z_ctry, na.rm = TRUE),
    mean_constr_z_ctry = mean(constr_z_ctry, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  arrange(desc(n_strong_brokers))

# Visualisation: k-core vs degree (within-country z-scores)
ggplot(ctry_struct_z, aes(x = mean_kcore_z_ctry, y = mean_deg_z_ctry, label = main_ctry)) +
  ...
  labs(
    x = "Mean k-core z-score (within-country)",
    y = "Mean degree z-score (within-country)",
    title = "Typical structural position of strong brokers by country"
  ) +
  theme_brokers()

# Cluster countries by broker structural profile
clust_df <- ctry_struct_z %>%
  select(main_ctry, mean_deg_z_ctry, mean_kcore_z_ctry, mean_constr_z_ctry) %>%
  drop_na() %>%
  column_to_rownames("main_ctry")

d_struct  <- dist(clust_df, method = "euclidean")
hc_struct <- hclust(d_struct, method = "ward.D2")

# KS distances between degree distributions
brokers_geo <- inventor_panel_geo %>%
  group_by(main_ctry) %>%
  mutate(deg_z_ctry = (deg - mean(deg, na.rm = TRUE)) / sd(deg, na.rm = TRUE)) %>%
  ungroup() %>%
  filter(is_broker_1sd)

ctrys <- sort(unique(brokers_geo$main_ctry))
n_c   <- length(ctrys)
ks_deg <- matrix(NA_real_, n_c, n_c, dimnames = list(ctrys, ctrys))

for (i in seq_along(ctrys)) {
  for (j in seq_along(ctrys)) {
    if (j <= i) next
    x <- brokers_geo$deg_z_ctry[brokers_geo$main_ctry == ctrys[i]]
    y <- brokers_geo$deg_z_ctry[brokers_geo$main_ctry == ctrys[j]]
    
    if (length(x) < 50 || length(y) < 50) next
    
    ks <- suppressWarnings(ks.test(x, y))
    ks_deg[i, j] <- ks$statistic
    ks_deg[j, i] <- ks$statistic
  }
}

max_ks <- max(ks_deg, na.rm = TRUE)
ks_deg_filled <- ks_deg
ks_deg_filled[is.na(ks_deg_filled)] <- max_ks

# KS heatmap
ks_long <- as.data.frame(ks_deg_filled) %>%
  rownames_to_column("ctry_row") %>%
  pivot_longer(-ctry_row, names_to = "ctry_col", values_to = "ks")

ggplot(ks_long, aes(x = ctry_col, y = ctry_row, fill = ks)) +
  geom_tile() +
  scale_fill_gradient(low = "#fff7ec", high = "#7f0000") +
  theme_brokers() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 0)) +
  labs(title = "KS distance between strong-broker degree distributions")
...
# ===================================================================== --
# 30. Comparing Broker Roles Across Countries ----
# ===================================================================== --

# Top 25 countries by number of strong brokers
top_countries <- broker_features_df %>%
  count(main_ctry) %>%
  arrange(desc(n)) %>%
  slice_head(n = 25) %>%
  pull(main_ctry)

# Role distribution for top countries
role_ctry_dist <- broker_features_df %>%
  filter(main_ctry %in% top_countries) %>%
  count(main_ctry, role_id) %>%
  group_by(main_ctry) %>%
  mutate(share = n / sum(n)) %>%
  ungroup()

# Order countries by prevalence of Role 1
ctry_ord <- role_ctry_dist %>%
  filter(role_id == "1") %>%
  arrange(desc(share)) %>%
  pull(main_ctry)

role_ctry_dist <- role_ctry_dist %>%
  mutate(main_ctry = factor(main_ctry, levels = ctry_ord))

# Plot
ggplot(role_ctry_dist, aes(x = main_ctry, y = share, fill = role_id)) +
  geom_col(position = "fill", width = 0.8) +
  scale_y_continuous(labels = percent, expand = c(0, 0)) +
  scale_fill_brewer(palette = "Set2", name = "Broker role") +
  labs(
    title    = "Composition of broker roles across countries",
    subtitle = "Share of strong brokers by role (top 25 countries)",
    x        = "Country",
...
# ===================================================================== --
# 31. Visual Validation: Ego Networks (1-step & 2-step) ----
# ===================================================================== --
...
plot_ego_1_2 <- function(role_name, g_full = coinv_net, seed = 2025) {
  ...
  plot(
    ...
    sub = paste0("Degree: ", ex_deg, "   K-core: ", ex_kcore, "   Constraint: ", round(ex_constr, 3))
  )
  ...
  plot(
    ...
    sub = paste0("Local clustering: ", round(ex_clust, 3), "   Avg neighbor degree: ", round(ex_neighdeg, 1))
  )
...
# ===================================================================== --
# 32. Descriptive Statistics for Broker Roles ----
# ===================================================================== --
...
role_table %>%
  kable("html", caption = "Descriptive Statistics of Broker Types", align = "lrrrrrrrrrrrr") %>%
...
# ===================================================================== --
# 33. Geographic Map: Dominant Broker Roles (NUTS2) ----
# ===================================================================== --
...
map_plot_data <- nuts2_map_ready %>%
  ...
  st_crop(xmin = 2500000, xmax = 7000000, ymin = 1500000, ymax = 5500000)

# Plot
ggplot() +
  geom_sf(data = countries_laea, fill = "grey95", color = "white", size = 0.2) +
  geom_sf(data = map_plot_data, aes(fill = dominant_role), color = "grey80", size = 0.1) +
...
# ===================================================================== --
# 34. Clustering Countries by Broker Mix ----
# ===================================================================== --
...
# Stacked bar: broker mix by country group
...
p_map_final <- ggplot() +
  geom_sf(data = bg_map, fill = "#f0f0f0", color = "white", size = 0.2) +
  geom_sf(data = map_data, aes(fill = cluster_name), color = "white", size = 0.05) +
  scale_fill_manual(
...
# ===================================================================== --
# 35. Country-level Summaries for Export ----
# ===================================================================== --
...
# ===================================================================== --
# 36. Maximum Spanning Tree (MST) Backbone Visualization ----
# ===================================================================== --

message("Generating Maximum Spanning Tree (MST) Map...")

# 1. Prepare the graph on NUTS2 edges
if (!exists("edges_tl2")) stop("edges_tl2 object not found. Run Part A first.")
...
# 3. Convert MST edges back to spatial lines
mst_edges_df <- igraph::as_data_frame(g_mst, what = "edges") %>%
  as_tibble() %>%
  select(from, to, weight)

if (!exists("centroids_full")) stop("centroids_full object not found. Run Part A first.")

nodes_geo <- centroids_full %>%
  st_transform(3035) %>%
  select(nuts_id, geometry)

# Helper: Build SF lines
build_lines <- function(df, nodes) {
  coords <- df %>%
    left_join(nodes, by = c("from" = "nuts_id")) %>%
    rename(geom_from = geometry) %>%
    left_join(nodes, by = c("to" = "nuts_id")) %>%
    rename(geom_to = geometry) %>%
    filter(!st_is_empty(geom_from), !st_is_empty(geom_to))
  
  lines_list <- map(1:nrow(coords), function(i) {
    pt1 <- st_coordinates(coords$geom_from[[i]])
    pt2 <- st_coordinates(coords$geom_to[[i]])
    st_linestring(rbind(pt1, pt2))
  })
  ...
  st_sf(
    ...
    geometry = st_sfc(lines_list, crs = st_crs(nodes))
  )
}

mst_sf <- build_lines(mst_edges_df, nodes_geo)

# 4. Weight tertiles for visualization
mst_sf <- mst_sf %>%
  mutate(
    tier_rank = ntile(weight, 3),
    tier_label = factor(
      case_when(
        tier_rank == 1 ~ "3. Peripheral ties",
        tier_rank == 2 ~ "2. Intermediate ties",
        tier_rank == 3 ~ "1. Backbone ties"
      ),
      levels = c("3. Peripheral ties", "2. Intermediate ties", "1. Backbone ties")
    )
  )

bg_map_mst <- gisco_get_countries(region = "Europe") %>% st_transform(3035)

p_mst <- ggplot() +
  geom_sf(data = bg_map_mst, fill = "#F0F0F0", color = "white", linewidth = 0.2) +
  geom_sf(
    data = mst_sf %>% arrange(tier_rank),
    aes(alpha = tier_label, linewidth = tier_label, color = tier_label)
  ) +
  scale_color_manual(
    values = c("grey70", "grey50", "#2b8cbe"),
    name   = "Connection strength"
  ) +
  scale_linewidth_manual(
    values = c(0.3, 0.6, 1.1),
    name   = "Connection strength"
  ) +
  scale_alpha_manual(
    values = c(0.4, 0.7, 1.0),
    name   = "Connection strength"
  ) +
  coord_sf(xlim = c(2600000, 6200000), ylim = c(1500000, 5400000), expand = FALSE) +
  labs(
    title    = "Backbone of the European co-inventor network",
    subtitle = "Maximum spanning tree (NUTS2-level collaboration ties)",
    caption  = "Edges are partitioned into tertiles by collaboration intensity."
  ) +
  theme_void(base_family = "Times New Roman") +
  theme(
    legend.position = "bottom",
...
# ===================================================================== --
# 37. Slide-Style MST Backbone Plot (Straight Edges) ----
# ===================================================================== --
